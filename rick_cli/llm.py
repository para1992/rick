        ...
        for attempt in range(1, attempts + 1):
            request = urllib.request.Request(
                ...
                headers={
                    ...
                    "X-Title": os.getenv("OPENROUTER_SITE_NAME", "Rick CLI"),
                },
                method="POST",
            )
...
def _response_format_for(messages: list[dict[str, str]]) -> dict[str, Any]:
    ...
    if "hidden judging rubric" in system_text or "define_dod" in system_text:
        return _json_schema_response_format("rick_dod", _dod_schema())

    if "candidate generation" in system_text:
        return _json_schema_response_format("rick_candidate", _candidate_schema())

    if "selected_index" in joined and "score" in joined and "reason" in joined:
        return _json_schema_response_format("rick_judge", _judge_schema())

    if "fallback_used" in joined and '"units"' in joined:
        return _json_schema_response_format("rick_explode", _explode_schema())
